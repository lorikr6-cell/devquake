...
/** The user manual: from the first setup through a workout to the calendar and photos. */
export default function Help({ ctx }: PluginPageProps) {
  const manual = MANUAL[localeOf(ctx)];
  const values = {
    host: (
  ...
  function block(b: ManualBlock, key: number): ReactNode {
    ...
    if ('h3' in b) {
      return (
        <h3 key={key} className="font-display text-lg font-semibold">
          {b.h3}
        </h3>
      );
    }
  ...
  return (
    <article className="mx-auto max-w-3xl space-y-10 leading-relaxed">
      ...
      <header>
        ...
        <h1 className="font-display text-3xl font-bold sm:text-4xl">{manual.title}</h1>
      ...
      {manual.sections.map((s) => (
        <section key={s.id} id={s.id} className="scroll-mt-32 space-y-3">
