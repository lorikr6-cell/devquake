...
import { storeTypesByCategory } from '../lib/store-types';
  ...
  function block(b: ManualBlock, key: number): ReactNode {
    ...
    return (
      <ul key={key} className="list-disc space-y-1 pl-5 text-sm">
        {storeTypesByCategory().map((g) => (
          <li key={g.group}>
            <strong>{t(`storeCategories.${g.group}`)}</strong>:{' '}
            {g.types.map((type) => t(`storeTypes.${type.code}.label`)).join(', ')}
          </li>
        ))}
      </ul>
    );
