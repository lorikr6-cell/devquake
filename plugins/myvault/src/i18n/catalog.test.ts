...
import { CATEGORIES, QUESTION_TYPES } from '../lib/model';
...
describe('myvault message catalogs', () => {
  it('names every category and question type', () => {
    for (const c of CATEGORIES) expect(typeof node(en, `categories.${c.code}`)).toBe('string');
    for (const q of QUESTION_TYPES) expect(typeof node(en, `questions.types.${q}`)).toBe('string');
  });
