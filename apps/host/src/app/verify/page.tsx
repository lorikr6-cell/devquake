...
export async function generateMetadata() {
  return { title: (await getT('auth.verify'))('metaTitle'), robots: { index: false } };
}

export default async function VerifyPage() {
  const challenge = await getPendingChallenge().catch(() => null);
  const t = await getT('auth.verify');
          ...
          {challenge ? (
            <>
              <p className="mt-2 mb-6 text-sm text-ink/70 dark:text-paper/70">
                {rich(t(challenge.purpose === 'signup' ? 'confirm' : 'finish'), {
                  email: <strong>{maskEmail(challenge.email)}</strong>,
