module.exports=[57458,e=>{"use strict";var i=e.i(29367),t=e.i(21513);let s=(0,i.api)(async({db:e,user:i})=>({fingerprint:await (0,t.changesFingerprint)(e,i.id)}));e.s(["GET",0,s])},29367,e=>{"use strict";var i=e.i(48161),t=e.i(63767);e.s(["api",0,function(e){return async(s,{params:r,ctx:n})=>{let d=(0,i.translator)((0,i.localeOf)(n));if(!n.user)return Response.json({error:d("errors.signIn")},{status:401});if(!n.db)return Response.json({error:d("errors.unavailable")},{status:503});try{let i=await e({request:s,params:r,db:n.db,user:n.user,people:n.people});if(i instanceof Response)return i;if(void 0===i)return new Response(null,{status:204});return Response.json(i,{headers:{"Cache-Control":"no-store"}})}catch(e){if(e instanceof t.HttpError){let{field:i,...t}=e.params,s=void 0===i?t:{...t,field:d(`fields.${i}`)};return Response.json({error:d(`errors.${e.key}`,s)},{status:e.status})}throw e}}}])},21513,63767,37699,28789,e=>{"use strict";var i=e.i(66680);class t extends Error{status;key;params;constructor(e,i,t={}){super(i),this.status=e,this.key=i,this.params=t}}e.s(["HttpError",0,t],63767);let s="ABCDEFGHJKMNPQRSTUVWXYZ23456789";function r(e){let i="";for(let t=0;t<8;t++)i+=s[e(s.length)];return i}function n(e,i,t){return`/api/lists/${e}/items/${i}/photo?v=${t}`}e.s(["CURRENCIES",0,["RON","EUR","USD","HUF","GBP"],"INVITE_CODE_PATTERN",0,/^[A-HJ-KM-NP-Z2-9]{8}$/,"newInviteCode",0,r],37699),e.s(["MAX_PHOTO_BYTES",0,2097152,"photoUrl",0,n,"sniffPhoto",0,function(e){if(e.length<12)return null;if(255===e[0]&&216===e[1]&&255===e[2])return"image/jpeg";if(137===e[0]&&80===e[1]&&78===e[2]&&71===e[3]&&13===e[4]&&10===e[5])return"image/png";let i=(i,t)=>String.fromCharCode(...e.slice(i,t));return"RIFF"===i(0,4)&&"WEBP"===i(8,12)?"image/webp":null}],28789);let d="DATE_FORMAT(l.shop_date, '%Y-%m-%d') AS shop_date";async function a(e,i,t){let[s]=await e.query(`SELECT l.id, l.name, l.currency, ${d}, l.version, m.role
      ...
      WHERE l.id = ? AND m.user_id = ?`,[i,t]);return s??null}async function l(e,i,s){let r=await a(e,i,s);if(!r)throw new t(404,"listNotFound");return r}async function o(e,i,s){let r=await l(e,i,s);if("owner"!==r.role)throw new t(403,"ownerOnly");return r}async function _(e,i){await e.execute("UPDATE lists SET version = version + 1 WHERE id = ?",[i])}async function u(e,i,t){await e.execute("UPDATE list_members SET display_name = ? WHERE list_id = ? AND user_id = ? AND display_name <> ?",[t.displayName,i,t.id,t.displayName])}async function m(e,i){return(await e.query(`SELECT l.id, l.name, l.currency, ${d}, m.role,
       ...
       FROM lists l JOIN list_members m ON m.list_id = l.id
      WHERE m.user_id = ?
      ORDER BY l.shop_date DESC, l.updated_at DESC`,[i])).map(e=>({id:e.id,name:e.name,currency:e.currency,shopDate:e.shop_date,role:e.role,members:Number(e.members),open:Number(e.open??0),done:Number(e.done??0),total:Math.round(100*Number(e.total??0))/100}))}let E=`i.id, i.list_id, i.store_id, i.name, i.quantity, i.unit, i.price,
  ...
  UNIX_TIMESTAMP(p.updated_at) AS photo_v`;function c(e){return{id:e.id,storeId:e.store_id,name:e.name,quantity:null===e.quantity?null:Number(e.quantity),unit:e.unit,price:null===e.price?null:Number(e.price),estimatedPrice:null===e.estimated_price?null:Number(e.estimated_price),priceCorrectedByName:e.price_corrected_by_name,description:e.description,addedByName:e.added_by_name,done:null!==e.done_at,doneByName:e.done_by_name,dropped:null!==e.dropped_at,droppedByName:e.dropped_by_name,photo:null===e.photo_v?null:n(e.list_id,e.id,Number(e.photo_v))}}async function p(e,i,t){let s=await l(e,i,t),[r,n,d]=await Promise.all([e.query(`SELECT user_id, display_name, role FROM list_members WHERE list_id = ?
      ...
      WHERE i.code = ? AND i.revoked_at IS NULL`,[i]);return t??null}async function S(e,i,s){let r=await R(e,i);if(!r)throw new t(404,"inviteInvalid");return(await e.execute("INSERT IGNORE INTO list_members (list_id, user_id, role, display_name) VALUES (?, ?, 'member', ?)",[r.id,s.id,s.displayName])).affectedRows>0&&(await e.execute("INSERT INTO list_events (list_id, user_id, user_name, kind) VALUES (?, ?, ?, 'member_joined')",[r.id,s.id,s.displayName]),await _(e,r.id)),r.id}async function T(e,i,t){let s=new Map;if(0===t.length)return s;let r=t.map(()=>"?").join(", "),[n,d]=await Promise.all([e.query(`SELECT s.list_id, s.id, s.name, s.type, s.location, s.description
         FROM stores s JOIN list_members m ON m.list_id = s.list_id AND m.user_id = ?
        WHERE s.list_id IN (${r}) ORDER BY s.name, s.id`,[i,...t]),e.query(`SELECT ${E}
         FROM items i JOIN list_members m ON m.list_id = i.list_id AND m.user_id = ?
         LEFT JOIN item_photos p ON p.item_id = i.id
        WHERE i.list_id IN (${r}) ORDER BY i.position, i.id`,[i,...t])]),a=e=>{let i=s.get(e);return i||(i={stores:[],items:[]},s.set(e,i)),i};for(let{list_id:e,...i}of n)a(e).stores.push(i);for(let e of d)a(e.list_id).items.push(c(e));return s}let I=`(SELECT list_id, user_id, display_name FROM list_members
   UNION ALL SELECT list_id, user_id, display_name FROM deleted_list_members)`;async function L(e,i){let t=`JOIN ${I} me ON me.list_id = x.list_id AND me.user_id = ?`,[s,r,n,a]=await Promise.all([e.query(`SELECT l.id, l.name, l.currency, ${d}, l.deleted_at IS NOT NULL AS deleted
      ...
      LIMIT 1500`,[i])).map(e=>({itemId:e.item_id,listId:e.list_id,shopDate:e.shop_date,name:e.name,unit:e.unit,quantity:null===e.quantity?null:Number(e.quantity),price:null===e.price?null:Number(e.price),description:e.description,storeName:e.store_name,storeType:e.store_type,storeLocation:e.store_location,storeDescription:e.store_description,photo:null===e.photo_v?null:n(e.list_id,e.item_id,Number(e.photo_v))}))}async function b(e,i){let[t]=await e.query(`SELECT COUNT(*) AS lists, SUM(l.version) AS versions
       FROM lists l JOIN list_members m ON m.list_id = l.id AND m.user_id = ?`,[i]);return`${Number(t?.lists??0)}:${Number(t?.versions??0)}`}async function f(e,i,t,s=20){return(await e.query(`SELECT e.id, e.list_id, l.name AS list_name, e.user_name, e.kind, e.item_name,
      ...
      ORDER BY e.id DESC
      LIMIT ${Math.max(1,Math.min(s,50))}`,[i,i,i,t??0,i])).map(e=>({id:Number(e.id),listId:e.list_id,listName:e.list_name,userName:e.user_name,kind:e.kind,itemName:e.item_name,at:e.at}))}async function D(e,i,t){await e.execute(`INSERT INTO notification_clears (user_id, cleared_up_to) VALUES (?, ?)
       ON DUPLICATE KEY UPDATE cleared_up_to = GREATEST(cleared_up_to, VALUES(cleared_up_to))`,[i,t]),await e.execute("DELETE FROM notification_dismissals WHERE user_id = ? AND event_id <= ?",[i,t])}async function C(e,i,t){await e.execute(`INSERT IGNORE INTO notification_dismissals (user_id, event_id)
     SELECT ?, e.id FROM list_events e
       JOIN list_members m ON m.list_id = e.list_id AND m.user_id = ?
      WHERE e.id = ?`,[i,i,t])}async function h(e,i){return(await e.query(`SELECT o.product, o.unit, l.currency, o.kind, o.price,
      ...
      LIMIT 5000`,[i])).map(e=>({product:e.product,unit:e.unit,currency:e.currency,kind:e.kind,price:Number(e.price),observedOn:e.observed_on,itemId:null===e.item_id?null:Number(e.item_id)}))}e.s(["activeInvite",0,y,"changesFingerprint",0,b,"clearEvents",0,D,"createList",0,N,"dismissEvent",0,C,"eventsForUser",0,f,"itemsOfLists",0,T,"joinByInvite",0,S,"listByInvite",0,R,"listsForUser",0,m,"membership",0,a,"priceHistory",0,h,"refreshMemberName",0,u,"requireMember",0,l,"requireOwner",0,o,"rotateInvite",0,O,"snapshot",0,p,"statsInput",0,L,"suggestionRows",0,A,"touch",0,_],21513)}];

//# sourceMappingURL=plugins_shopping_src_0ot0ck1._.js.map